...
| 阶段 | 内容 | 状态 |
|---|---|---|
| 骨架 | 工作区 + 共享类型 + 管理器 API 骨架 + UI 看板占位 + Electron 壳 | ✅ 当前 |
| M0 | 冒烟验证：同一台机器同时拉起 2 个不同版本实体，确认独立性 | ⏳ 下一步 |
| M1 | 核心循环：版本仓库安装 + 实体 spawn/stop + 最简创建向导 | |
| M2 | 实体详情（内嵌 GUI + 日志流）、模型配置、端口分配 | |
| M3 | 版本切换 / 升级快照 / 回滚、实体导入导出 | |
